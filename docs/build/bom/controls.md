...
| Exhaust / circulation fans | **Brushless/EC DC *axial***, **Backwoods 16" 12/24 V** (36 W → 1627 CFM ≈ **45 CFM/W**) *or equal*; **solar-direct** | ~2 | REQ-COOL-5 | TBD | TBD | TBD | TBD | proposed | **Select on CFM/W** (watts = recurring battery cost), target ~30–50. **Affinity laws: bigger diameter + slower RPM = far better CFM/W** (P ∝ RPM³) — oversize, run slow (also quieter). Axial for low-static paths |
...
| **Per-circuit indicator lamps** | **Metal IP-rated panel lamp** after each fuse/breaker, **matched voltage per rail** (DC **5 V / 12 V / 24 V / 48 V** — *e.g.* **Gebildet 3–9 V** (5 V/logic) · 12–24 V dash lamp · **PL8B 48 V**; **don't cross-use**); **standardize one barrel size** (6 vs 8 mm) for a uniform panel; **color-coded by rail/tier**; + **blown-fuse LED**; optional **controller-driven status LED** (green on / amber shed / red fault) off ESP32 GPIO | TBD | REQ-NET-7, REQ-EDU-4, REQ-CTRL-1 | TBD | TBD | TBD | TBD | proposed | "Is it live?" at a glance — Amber diagnostic + teaching display; mW. **Not Shelly**. Tier-1 indication stays **passive** |
...
| Desiccant + breather vent | Enclosure moisture control; **reusable** — regenerated by solar (no discard) | TBD | REQ-CTRL-5, REQ-CYCLE-9 | TBD | TBD | TBD | TBD | proposed | Pairs w/ breather membrane; solar-regen = produce-no-waste + lesson. Oven ↓ |
| Solar regen cooker | **Parabolic solar cooker (~1.5 m, ~1800 W)** *or equal* | 1 | REQ-CYCLE-9, REQ-SAFE-4 | TBD | TBD | TBD | TBD | proposed | Regenerates desiccant + dries seeds + BBQ demo. ⚠️ **Concentrated-solar hazard: 🔴 adult/Red-only, supervised, controlled siting, defocus+stow when unattended** |
