...
| REQ-CYCLE-3 | SHOULD | **Three water elevations** — white + pure **HIGH** (tower; head to sinks), grey reuse buffer **MEDIUM** (gravity-collects returns, gravity-feeds beds), bulk cistern **LOW**. Tower engineered ([REQ-STR-1](#a-structure--envelope)); tanks **covered** ([REQ-WATER-2](#e-irrigation--water)); tether fills white via **air-gap** ([REQ-WATER-4](#e-irrigation--water)). | Resilience, Safety |
...
| REQ-CYCLE-13 | SHOULD | **Catch high + managed waterfall spill.** Capture returns (sink/fogger) at source height; gravity-cascade to the medium grey buffer. **Managed waterfall spill** = buffer (with optionality valves) **fills → spills bed-to-bed when full → drain-line emitters → high-volume bypass to daylight only when everything is saturated.** Pump-free, zero controls; size buffer for turnover (no stagnant greywater). | Off-grid, Resilience |
| REQ-CYCLE-14 | SHOULD | **Chainable, overflow-limited beds.** Each bed has a **dedicated inlet + a dedicated outlet = overflow standpipe set at reservoir top** (water line set by standpipe, not fill volume → can't overfill the bed top). Beds chain **outlet(N)→inlet(N+1)** as a one-way waterfall (no bottom-to-bottom manifold — avoids communicating-vessel level equalization + back-siphon). Stair-step beds or step standpipe heights down to keep head within the ~12" catch→bed band ([REQ-PASV-4](#m-passive-architecture)); last bed's overflow → sloped drain-line emitters → high-volume bypass to daylight. | Off-grid, Resilience |
