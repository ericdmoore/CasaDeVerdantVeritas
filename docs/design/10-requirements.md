...
| REQ-CYCLE-13 | SHOULD | **Catch high + managed spill.** Capture returns (sink/fogger) at source height; gravity-cascade to the medium grey buffer. **Managed spill** = buffer (with optionality valves) **fills → spills to beds when full → swale only when beds saturated.** Pump-free; size buffer for turnover (no stagnant greywater). | Off-grid, Resilience |
