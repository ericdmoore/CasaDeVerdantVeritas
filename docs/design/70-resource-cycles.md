...
**Solar desiccant regeneration (reuse loop).** Enclosure desiccant (silica gel) is **reused, not discarded** — regenerated by solar heat (off-grid, free) in the outdoor service zone, then put back. Produce-no-waste + a solar-thermal lesson; also dries seeds/herbs and doubles as a cooking/BBQ showcase demo.
**Device: parabolic solar cooker (~1.5 m, ~1800 W)** *or equal* — hotter/faster than a box oven.
> ⚠️ **Serious concentrated-solar hazard — 🔴 adult/Red-only, never kid-operated** ([REQ-SAFE-4](10-requirements.md#g-safety--egress)). The focal point **ignites and burns**; the glare is an **eye hazard**. Controls: **access-limited siting** (service zone), flammables clear, non-combustible base, **supervised use only**, and **defocus + stow the reflector when unattended** (the focus tracks the sun). A higher safety bar than a box oven — accepted for the capability + demos.
