...
- **Power: off-grid by default.** Solar + battery, no permanent utility connection. The design must **lean on passive systems to survive the worst case — a hot *and* cloudy multi-day summer stretch** — when cooling demand peaks while solar generation and battery reserves bottom out. Powered systems are an assist, not the survival path. Two refinements: (a) an **emergency-only campus tether** (temporary extension cord + garden hose) is a last-resort backstop for extreme weather — explicitly *not* a dependency, kept temporary so it never becomes licensed/permanent work; (b) the battery is **modular and mobile** — a small fixed Tier-1 reserve stays put while a wheeled bulk pack (the "power cow") doubles as event power. The three resilience layers: **passive (always) → solar+battery (normal) → campus tether (emergency).** *(Decided 2026-06-08.)*
